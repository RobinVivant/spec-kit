...
# Supports: CLAUDE.md, GEMINI.md, .github/copilot-instructions.md, and OPENCODE.md
...
NEW_LANG=$(grep -F "**Language/Version**: " "$NEW_PLAN" 2>/dev/null | head -1 | sed 's/^\*\*Language\/Version\*\*: //' | grep -F -v "NEEDS CLARIFICATION" || echo "")
NEW_FRAMEWORK=$(grep -F "**Primary Dependencies**: " "$NEW_PLAN" 2>/dev/null | head -1 | sed 's/^\*\*Primary Dependencies\*\*: //' | grep -F -v "NEEDS CLARIFICATION" || echo "")
NEW_TESTING=$(grep -F "**Testing**: " "$NEW_PLAN" 2>/dev/null | head -1 | sed 's/^\*\*Testing\*\*: //' | grep -F -v "NEEDS CLARIFICATION" || echo "")
NEW_DB=$(grep -F "**Storage**: " "$NEW_PLAN" 2>/dev/null | head -1 | sed 's/^\*\*Storage\*\*: //' | grep -F -v "N/A" | grep -F -v "NEEDS CLARIFICATION" || echo "")
NEW_PROJECT_TYPE=$(grep -F "**Project Type**: " "$NEW_PLAN" 2>/dev/null | head -1 | sed 's/^\*\*Project Type\*\*: //' || echo "")
...
case "$AGENT_TYPE" in
    ...
    "")
        ...
        [ -f "$OPENCODE_FILE" ] && update_agent_file "$OPENCODE_FILE" "OpenCode"
        ...
        ;;
    *)
        echo "ERROR: Unknown agent type '$AGENT_TYPE'. Use: claude, gemini, copilot, opencode, or leave empty for all."
        exit 1
        ;;
esac
echo ""
...
echo ""
echo "Usage: $0 [claude|gemini|copilot|opencode]"
...
echo "  - opencode: Update only OPENCODE.md"
